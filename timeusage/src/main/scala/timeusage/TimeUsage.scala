...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(StructField(columnNames.head, StringType, nullable = false) :: columnNames.tail.map(colName => StructField(colName, DoubleType, nullable = false)))

  /** @return An RDD Row compatible with the schema produced by `dfSchema`
    * @param line Raw fields
    */
  def row(line: List[String]): Row = Row.fromSeq(line.head :: line.tail.map(s => s.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val primary = columnNames.collect({ case colName if startsWith(colName, "t01", "t03", "t11", "t1801", "t1803") => col(colName) })
    val working = columnNames.collect({ case colName if startsWith(colName, "t05", "t1805") => col(colName) })
    val other   = columnNames.collect({ case colName if startsWith(colName, "t02", "t04", "t06", "t07", "t08", "t09", "t10",
                                                                            "t12", "t13", "t14", "t15", "t16", "t18") &&
                                                       !startsWith(colName, "t1801", "t1803", "t1805")  => col(colName) })
  ...
  /** @return SQL query equivalent to the transformation implemented in `timeUsageGrouped`
    ...
    */
  def timeUsageGroupedSqlQuery(viewName: String): String = "SELECT working, sex, age, ROUND(AVG(primaryNeeds), 1) AS primaryNeeds, ROUND(AVG(work), 1) AS work, ROUND(AVG(other), 1) AS other " +
                                                           "FROM " + viewName + " " +
...
//  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(row => (row.working, row.sex, row.age))
          ...
          .map( { case ((working, sex, age), primaryNeeds, work, other) => TimeUsageRow(working, sex, age, Math.round(primaryNeeds*10)/10.0, Math.round(work*10)/10.0, Math.round(other*10)/10.0) })
          .orderBy("working", "sex", "age")
  }
}
